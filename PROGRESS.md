...
2. **Theme palettes:** per-face (default; faithful to the showcase, v1 editor shows each face's picker) vs one shared unified palette (cleaner editor, departs from showcase). Thin pass doesn't depend on it.
...
3. **Resolve the two OPEN decisions** (see the "## OPEN" section): (a) per-face complication gating (List string-compare likely unviable → accept-bleed / Boolean model / probe / share-slots); (b) theme palettes per-face vs shared (default per-face). Both raised with the owner.
