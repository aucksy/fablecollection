...
## OPEN — decisions to resolve before fleshing out Pulsar (raised with owner 2026-07-10)
1. **Per-face complication gating.** The `[CONFIGURATION.face] == 'id'` string compare is likely NOT viable at runtime (review Finding 1 + no v1 string-literal grammar). Options: (a) accept the Pulsar complication-bleed short-term; (b) model "face" as a **BooleanConfiguration** so the gate is the PROVEN `[CONFIGURATION.face] ? 0 : 255` — clean + safe, but caps at 2 faces (contradicts the List/5-face plan, needs migration for face 3); (c) test the compare on-wrist with a throwaway probe first; (d) faces share the Scene's ≤8 slots. Owner to weigh List-scalability vs proven-gating.
