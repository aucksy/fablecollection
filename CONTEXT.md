...
5. Current phase: getting Solstice (ARCLIGHT face 01) CI-green and on-wrist, then a design-fidelity pass against the showcase, then face 02 (PULSAR) — which is a SECOND FACE INSIDE the Arclight APK, not a new project (each category APK carries its 5 faces; packaging-5-faces-per-WFF-APK is an open decision, see PROGRESS.md).
